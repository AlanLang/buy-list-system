...
layui.define(['layer','element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if (tabIndex === -1) {
			...
			var pageid = ELEM.titleBox.find('li').length;
			//添加tab
			...
			if (that.config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					element.tabDelete(ELEM.tabFilter,pageid).init();
				});
			};
			//切换到当前打开的选项卡
			element.tabChange(ELEM.tabFilter, pageid);
		} else {
			element.tabChange(ELEM.tabFilter, tabIndex);
		}
	};
	Tab.prototype.tabDelete = function (f, d) {
	    var e = $(".layui-tab[lay-filter=" + f + "]"),
	     self = e.children(".layui-tab-title").find("li.layui-this"),
         tit = e.children(".layui-tab-title").children("li"),
	    l = tit.index($(self));
	    element.tabDelete(f, l).init();
	}
